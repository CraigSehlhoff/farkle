...
export function heldFiveOfAKind(diceValue: DiceValue) {
  const { counts, isEmpty } = initializeCounts(diceValue);
  if (isEmpty) return 0;

  const heldFiveOfAKindValue = counts.findIndex(
    (count, index) =>
      count === 5 &&
      diceValue.filter((die) => die.value === index && die.held).length === 5
  );
  if (heldFiveOfAKindValue > 0) {
    return 2000;
  }
  return 0;
}
